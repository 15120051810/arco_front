...
export function setRouteEmitter(to: RouteLocationNormalized) {
  ...
  emitter.emit(key, to);
  console.log(filePath,'路由改变emit触发-->key', key, '->to',to)
...
export function listenerRouteChange(
  handler: (route: RouteLocationNormalized) => void, // 你传入的回调函数，每当路由改变时就会触发，接受 RouteLocationNormalized 类型参数的函数，并且不返回任何值。
  ...
  console.log(filePath,'listenerRouteChange监听到路由改变，订阅消息emitter.on', key, latestRoute)
